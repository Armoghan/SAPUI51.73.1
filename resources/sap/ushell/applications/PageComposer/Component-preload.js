...
sap.ui.define([
    ...
    return Controller.extend("sap.ushell.applications.PageComposer.controller.BaseController", {
        ...
        preview: function (sPageId) {
            if (sap.ushell.Container.getDirtyFlag()) {
                ...
                Promise.all([
                    ...
                    var oURLParameters = oURLParsingService.parseParameters(window.location.search);
                    oURLParameters["sap-ushell-xx-overwrite-config"] = ["ushell/spaces/enabled:true"];
                    var sURLParameters = oURLParsingService.paramsToString(oURLParameters);

                    var sOpenFLPPageHash = oCANService.hrefForExternal({
                        target: {
                    ...
                    var sPreviewUrl = window.location.pathname + "?" + sURLParameters + sOpenFLPPageHash;
         ...
         * @param {string} pageId The pageId
         * @param {string} [roleId] The role by which to scope the catalogs
         * @returns {ContentCatalogs} Contains the titles of each catalog, the tiles of each catalog and a map of vizIds to catalogtileData
         * @private
         */
        fetchCatalogInfo: function (pageId, roleId) {
            return this.getPageRepository().getCatalogs(pageId, roleId).then(function (catalogs) {
                var aCatalogs = catalogs || [],
                    aVisualizations = [],
                    mVisualizations = {},
                    aCatalogTitles = [],
                    aCatalogTiles = [],
...
sap.ui.define([
    ...
    return BaseController.extend("sap.ushell.applications.PageComposer.controller.BaseDialog.controller", {
        ...
        /**
         * Pre-filter available packages by the page ID namespace.
         * If a namespace is detected in the page ID, it is copied to the package input field if it is empty.
         * Does nothing if the package input field is not empty or if the dialog is not enhanced with transport information.
         *
         * @param {string} sId A possible page ID, to check if it contains a namespace and extract it.
...
sap.ui.define([
    ...
    "sap/ui/model/json/JSONModel",
    "sap/ushell/Config"
], function (mobileLibrary, JSONModel, Config) {
    "use strict";
    ...
    /**
     * Returns the model relevant indicies from the given visualization
     *
    ...
    /**
     * Returns the tileType and the tile properties respective of the tileType
     * @param {object} oContextInfo The tile data
     * @returns {{tileType: string, properties: object}} The result object
     * @private
     */
    function _getTileProperties (oContextInfo) {
        var oResult = {
            tileType: "sap.ushell.ui.tile.StaticTile",
            properties: {
                chipId: "",
                title: oContextInfo.title,
                subtitle: oContextInfo.subTitle,
                info: "",
                icon: oContextInfo.iconUrl,
                targetURL: ""
            }
        };
        switch (oContextInfo.tileType) {
            case "STATIC":
                return oResult;
            case "DYNAMIC":
                oResult.tileType = "sap.ushell.ui.tile.DynamicTile";
                oResult.properties.icon = "";
                return oResult;
            default:
                // Custom Tile also rendered as StaticTile
                oResult.properties.info = "[" + resources.i18n.getText("Title.CustomTile") + "]";
                oResult.properties.icon = "";
                return oResult;
        }
    }
    ...
    return {
         ...
         * @param {sap.ui.model.Context} oContext The data for the specific visualization
         * @returns {sap.ushell.ui.launchpad.VizInstance} A visualization inside of a section
         *
         * @private
         */
        visualizationFactory: function (sId, oContext) {
            if (!oMainController.oVisualizationLoadingService) {
                throw new Error("Visualization Service was not loaded yet!");
            }

            var oContextInfo = oContext.getProperty(oContext.sPath),
                sVizId = oContextInfo.vizId,
                oTileProperties = _getTileProperties(oContextInfo),
                oVisualization = oMainController.oVisualizationLoadingService.instantiateVisualization({
                    vizId: sVizId,
                    previewMode: true,
                    directLoad: true,
                    tileType: oTileProperties.tileType,
                    properties: oTileProperties.properties
                });
                ...
                oSection.getVisualizations().forEach(function (oVisualization, iVisualizationIndex) {
                    if (oVisualization.getState() === LoadState.Failed) {
                        ...
                        aWarnings.push({
                            ...
                            description: resources.i18n.getText("Message.NavTargetResolutionError", oVisualization.getTitle() || oVisualization.getCatalogTile().getTitle())
                        });
                    }
                });
            });
...
sap.ui.define([
    ...
    return function () {
        ...
        /**
         * Format the type of a visualization.
         *
         ...
         * @private
         */
        this.formatTileType = function (vizType) {
            if (vizType === "STATIC") {
                return resources.i18n.getText("Title.StaticTile");
            } else if (vizType === "DYNAMIC") {
                return resources.i18n.getText("Title.DynamicTile");
            } else if (vizType) {
                return resources.i18n.getText("Title.CustomTile");
            }
        ...
        var _addTiles = function () {
            ...
            if (oAddSingleTileItem) {
                aTileData = [oAddSingleTileItem.getBindingContext("catalogs").getProperty()]; // adds a single tile (from its own "Add" button)
            } else {
...
sap.ui.define([], function () {
     ...
     * @private
     */
    var PagePersistence = function (oDataModel, oResourceBundle) {
     ...
     * @param {string} pageId The pageId
     * @param {string} [roleId] The role by which to scope the catalogs
     * @returns {array} An array of catalogs
     *
     * @private
     */
    PagePersistence.prototype.getCatalogs = function (pageId, roleId) {
        return roleId
            ? this._getCatalogsByRole(roleId)
    ...
    PagePersistence.prototype._rejectWithErrorMessage = function (oError) {
        ...
        if (oError.statusCode === "412") {
            sErrorMessage = this._oResourceBundle.getText("Message.PageIsOutdated");
        } else {
            try {
     ...
     *
     * @param {string} roleId The role to scope by
     * @returns {Promise<{
                        catalogs: Array,
                        visualizations: Array
                    }>} A promise resolving to an object containing the catalogs and the visualizations
     * @private
     */
    PagePersistence.prototype._getCatalogsByRole = function (roleId) {
        return new Promise(function (resolve, reject) {
            this._oODataModel.read("/roleSet('" + encodeURIComponent(roleId) + "')", {
                urlParameters: {
